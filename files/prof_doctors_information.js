...
function getlist() {
    ...
    data ="hel";
    xhr.open("GET", "prof_main.php?q=" + data, true);
    xhr.onload = function () {
        ...
        show_doctors_list(resp[0],resp[1],resp[2],resp[3],resp[4],resp[5],resp[6],resp[7],resp[8]);
    };
...
function show_doctors_list(name,spec,number,online_pay,exp_y,address,phone,first_turn,week_days) {
    name_f =  name;
    spec_f = spec;
    number_f = number;
    ...
    for (let i=0;i<name.length;i++) {
        // آدرس
        document.getElementsByClassName("dl_left_span")[i].innerText = address[i];
